  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const [existing] = await tx
        ...
        .where(and(eq(collectionItems.collectionId, id), eq(collectionItems.itemId, parsedBody.itemId)))
        .for('update');
  ...
  try {
    await db.transaction(async (tx) => {
      const [source] = await tx
        .select()
        .from(collections)
        .where(and(eq(collections.id, parsedBody.sourceCollectionId), eq(collections.userId, userId)))
        .for('update');
      ...
      const [target] = await tx
        .select()
        .from(collections)
        .where(and(eq(collections.id, parsedBody.targetCollectionId), eq(collections.userId, userId)))
        .for('update');
